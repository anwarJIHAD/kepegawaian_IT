					...
					<div style="margin-bottom: 20px;">
						<div class="row">
							<?php if ($pegawai['role'] == 'Admin' || $pegawai['role'] == 'guru' || $pegawai['role'] == 'pustakawati') { ?>
								...
								<a href="<?= base_url() ?>PengajuanIzin/tambahizin" class="btn btn-outline-warning"><i
										class="bi bi-plus-circle"></i> Ajukan Perizinan</a>
								<?php if ($pegawai['role'] == 'Admin') { ?>
									<a href="<?= base_url() ?>PengajuanIzin/export" class="btn btn-outline-success"
									...
									<tr>
										...
										<td>
											<?php if ($pegawai['role'] == $this->session->userdata('role') && $us['role'] == $this->session->userdata('role') && $us['status'] != 'Diterima' && $us['status'] != 'Ditolak') { ?>
												<a href="<?= base_url('PengajuanIzin/hapus/') . $us['id_izin']; ?>" class="btn btn-danger btn-sm"><i
